...
class StravaConverter(Converter):
    ...
    def __init__(self, dir_in, lang="ENG", dir_out=None):
        """Main constructor for StravaConverter
        Parameters:
            dir_in (str): path to main strava data download folder
            lang (str): language of Strava data download (default is English)
            dir_out (str): path to directory to save the converted FIT files to.
        ...
        if lang not in ["ENG", "GER"]:
            raise ValueError("lang must be either 'ENG' or 'GER'")
        else:
            self._lang = lang
    ...
    def strava_fit_to_gpx(self):
        ...
        for f_activity in fit_files:
            ...
            md = df_acts.loc[df_acts[fn].str.contains(f_activity)].iloc[0, :].to_dict()
            ...
            if self._lang == "ENG":
                ...
                act_id = md["Aktivitäts-ID"]
                act_name = md["Name der Aktivität"]
                act_desc = md["Aktivitätsbeschreibung"]
